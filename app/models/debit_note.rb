class DebitNote < ActiveRecord::Base
  ...
  with_options touch: true do |assoc|
	...
	# has_many :debit_note_details, :dependent => :destroy
	# accepts_nested_attributes_for :debit_note_details, :reject_if => lambda { |a| a[:description].blank? || a[:description].nil? || 
	# 	a[:amount].blank? || a[:amount].nil? || a[:volume].blank? || a[:volume].nil? }, :allow_destroy => true
	with_options dependent: :destroy do |assoc|
    assoc.has_many :debit_note_details
	
    assoc.has_many :invoice_details, class_name: "DebitNoteDetail", inverse_of: :debit_note
    
    assoc.has_many :close_payment_histories, class_name: "ClosePayment", foreign_key: 'invoice_no', primary_key: 'no_dbn'
    
    assoc.has_one :invoice_inquiry, -> { where(is_shadow: false) }, class_name: "ClosePayment", foreign_key: 'invoice_no', primary_key: 'no_dbn'
	...
	with_options allow_destroy: true do |nest_attr|
  	nest_attr.accepts_nested_attributes_for :debit_note_details, :reject_if => lambda { |a| a[:description].blank? }
  ...
  scope :recent, -> {
    # references(:debit_note_details, bill_of_lading: {shipping_instruction: [:shipper, :vessels, si_containers: [:container]]})
    # .includes(:debit_note_details, bill_of_lading: {shipping_instruction: [:shipper, :vessels, si_containers: [:container]]})
    order(no_dbn: :desc)
  }
  scope :recent_with_vessels, -> {
    # references({bill_of_lading: {shipping_instruction: [:vessels]}}, :debit_note_details)
    # .includes({bill_of_lading: {shipping_instruction: [:vessels]}}, :debit_note_details)
  ...
  scope :reports, -> {
    references({shipping_instruction: [:vessels]}, :debit_note_details)
    .includes({shipping_instruction: [:vessels]}, :debit_note_details)
  ...
  search_syntax do
    search_by :text do |scope, phrases|
      columns = [:no_dbn, :currency_code, :customer_ori, :to_shipper, :destination,
	...
	validates_uniqueness_of :no_dbn, :message => "must be unique"
  ...
  def payment_note
    ...
    if shipping_instruction
      ...
      year = Date.today.strftime("%y")
		...
		# inv_count = DebitNote.where("YEAR(created_at) = ?", Date.today.year).count
		# year = Date.today.year.to_s[2..3]
    inv_count = DebitNote.where("no_dbn LIKE ?", "IBLDBN#{year}%").count

		if(inv_count == 0)
			total = 10001.to_s

			new_number = "IBLDBN#{year}" + total[1..total.length-1]
		else
			sum = 10000
      last_invoice = DebitNote.where("no_dbn LIKE ?", "IBLDBN#{year}%").order(no_dbn: :asc).last.no_dbn
			# last_invoice = DebitNote.where("YEAR(created_at) = ?", Date.today.year).last.no_dbn
